...
Game::Game(int player_num):
    ...
    pause(false)
{
    line_clear = new QMediaPlayer();
    line_clear->setVolume(100);
    ...
    if(player_num == 2) {
        map->player[0]->predict = masterDialog->player1;
        map->player[1]->predict = masterDialog->player2;
        connect(this,
        ...
        map->player[0]->predict = masterSDialog->player1;
        connect(this,
                SIGNAL(to_refresh(std::vector<Loc>*,std::vector<Loc>*)),          //连接信号和槽
                masterSScene,
                SLOT(refresh(std::vector<Loc>*,std::vector<Loc>*)));
...
void Game::drop_and_refresh() {                                                   //用于接收QTimer的信号
    if(!map->live()) {                                                            //如果判断死亡，返回得分，发射死亡信号
        QMediaPlayer Dead;
        Dead.setMedia(QUrl("qrc:/new/prefix1/QT resourse/dead.wav"));
        Dead.setVolume(80);
        Dead.play();
